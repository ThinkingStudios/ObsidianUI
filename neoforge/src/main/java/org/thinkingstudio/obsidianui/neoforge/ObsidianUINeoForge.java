...
public class ObsidianUINeoForge {
    public ObsidianUINeoForge() {
        if (FMLLoader.getDist().isClient()) {
            ...
            NeoForge.EVENT_BUS.addListener(EventPriority.HIGHEST, OpenScreenCallbackEvent.Post.class, event -> {
                HudManager.initAll(event.getClient(), event.getClient().getWindow().getScaledWidth(), event.getClient().getWindow().getScaledHeight());
            });
            NeoForge.EVENT_BUS.addListener(EventPriority.HIGHEST, ResolutionChangeCallbackEvent.class, event -> {
                HudManager.initAll(event.getClient(), event.getClient().getWindow().getScaledWidth(), event.getClient().getWindow().getScaledHeight());
            });
        }
    }
